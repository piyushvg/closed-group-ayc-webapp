...
export const FRAPPE_METHODS = {
  ...
  // ⚠️ CONFIRM: the Postman collection has an "Update Member API" but we
  // haven't pinned its exact method name from the web side yet. The RN app's
  // src/api/members.ts calls the real one — copy that path here.
  updateMember: `${PREFIX}.members_api.update_member_profile`,

  // Two-step photo flow, same as the RN app's src/api/photo.ts.
...
export async function frappePost(path, body) {
  ...
  try {
    ...
    throw new Error(`Backend returned non-JSON (HTTP ${res.status}) for ${path}`);
  }
