...
//
// `isBoardMember` rides along in the same signed payload. It is decided once
// at login rather than looked up on every request, and because the payload is
// signed it cannot be flipped to true from devtools.
//
...
export async function clearSessionCookie() {
  const store = await cookies();
  // Path must match the one set above, or the browser keeps the cookie.
